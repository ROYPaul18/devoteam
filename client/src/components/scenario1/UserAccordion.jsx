...
const UserAccordion = ({ user, loading }) => {
  ...
  return (
    <div className="p-2 2xl:p-5 border-solid solid border-b-2 border-secondary">
      {loading ? (
        ...
        <div className="flex justify-between ">
          <div onClick={() => setAccordionOpen(!accordionOpen)} className="flex-cols ">
            <p className="text-purpleRed-200 font-black mr-1 text-xl md:text-lg sm:text-base">
            ...
            <p className="text-purpleRed-100 font-bold text-lg md:text-base sm:text-sm">
          ...
          <button onClick={() => setAccordionOpen(!accordionOpen)}>
            {loading ? <Skeleton width={20} baseColor="#FFE1E1" highlightColor="#FFE1E1"/> : (
              accordionOpen ? (
                <span className="text-secondary text-sm md:text-xs sm:text-xs">-</span>
              ) : (
                <span className="text-secondary text-sm md:text-xs sm:text-xs">+</span>
              )
            )}
      ...
      {loading ? (
        ...
        <div
          className={`grid overflow-hidden transition-all duration-300 ease-in-out text-slate-600 text-sm md:text-xs sm:text-xs ${
            accordionOpen ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"
          }`}
        >
          <li className="overflow-hidden text-sm md:text-xs sm:text-xs">
